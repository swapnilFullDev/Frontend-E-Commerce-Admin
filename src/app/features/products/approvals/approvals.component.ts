...
export class RejectModalComponent {
  ...
  onReject(): void {
    this.isLoading = true;
    this.productService.rejectProduct(this.data.inventoryMasterId, this.rejectForm.get('remark')?.value || '').subscribe({
      next: () => {
...
export class ApprovalsComponent implements OnInit {
  ...
  rejectProduct(product: Product): void {
    ...
    dialogRef.afterClosed().subscribe(result => { this.loadPendingProducts(); });
  }
